...
def extract_group_id(html: str, group_name_norm: str) -> str | None:
    # F4 / elimination rounds keep the verCalendario(...) call but inside an
    # HTML comment, so a plain BS4 walk over live siblings misses it. We scan
    # the raw HTML slice between this group's <h3> and the next div.categoria
    # so the regex catches both live onclick attributes and commented-out ones.
    soup = BeautifulSoup(html, "lxml")
    for header in soup.select("div.categoria h3"):
        if norm_text(header.get_text()) != group_name_norm:
            continue
        h3_str = str(header)
        start = html.find(h3_str)
        if start == -1:
            continue
        next_cat = re.search(r'<div[^>]*class="[^"]*\bcategoria\b', html[start + len(h3_str):])
        end = (start + len(h3_str) + next_cat.start()) if next_cat else len(html)
        section = html[start:end]
        match = re.search(r"verCalendario\('([a-f0-9]+)'\)", section)
        if match:
...
def parse_week_jornada(html: str) -> list[tuple[str, JornadaMatchEntry]]:
    ...
    for node in nodes:
        ...
        if node.find_parent("div", class_="container2") or node.find_parent("div", class_="container"):
            continue
        # Find the immediately following sibling div.containerJugadoresDestacados,
        # skipping over whitespace/NavigableString nodes.
        top_scorer_widget: Tag | None = None
        sibling = node.next_sibling
        while sibling is not None:
            if isinstance(sibling, Tag):
                if "containerJugadoresDestacados" in (sibling.get("class") or []):
                    top_scorer_widget = sibling
                break
            sibling = sibling.next_sibling
        entry = _parse_match_block(node, top_scorer_widget)
        if entry and current_group:
            result.append((current_group, entry))
    return result


def _parse_top_scorer(widget: Tag) -> dict | None:
    """Extract the leading top scorer from a div.containerJugadoresDestacados.

    The widget contains one or more div.player elements, each with an onclick
    attribute of the form:
        openModal('dorsal','name','pts','t2','t3','tl_made','tl_made/tl_att','logo_url')

    We parse only the first entry (the highest scorer) and return a dict with
    keys: dorsal, name, pts, t2, t3, tl_made, tl_att, logo_url.
    Returns None if the pattern is not found.
    """
    _OPEN_MODAL_RE = re.compile(
        r"openModal\(\s*'([^']*)'\s*,\s*'([^']*)'\s*,\s*'([^']*)'\s*,"
        r"\s*'([^']*)'\s*,\s*'([^']*)'\s*,\s*'([^']*)'\s*,"
        r"\s*'([^']*)'\s*,\s*'([^']*)'\s*\)"
    )
    for player_div in widget.find_all("div", class_="player"):
        onclick = player_div.get("onclick", "")
        m = _OPEN_MODAL_RE.search(onclick)
        if not m:
            continue
        dorsal, name, pts_s, t2_s, t3_s, tl_made_s, tl_frac, logo_url = m.groups()
        try:
            pts = int(pts_s)
            t2 = int(t2_s)
            t3 = int(t3_s)
            tl_made = int(tl_made_s)
            tl_att_match = re.match(r"(\d+)\s*/\s*(\d+)", tl_frac)
            tl_att = int(tl_att_match.group(2)) if tl_att_match else 0
...
def _parse_match_block(block: Tag, top_scorer_widget: Tag | None = None) -> JornadaMatchEntry | None:
    ...
    top_scorer = _parse_top_scorer(top_scorer_widget) if top_scorer_widget is not None else None
